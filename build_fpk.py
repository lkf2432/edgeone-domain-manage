...
def copy_source():
    """复制 Python 源码和 Docker 相关文件到 fpk_build/app/docker/"""
    step(1, 4, "复制源码和 Docker 文件...")
    ...
    # Dockerfile 和 docker-compose.yaml 已在 docker_dir 中
    print("  -> Dockerfile (已存在)")
    print("  -> docker-compose.yaml (已存在)")
    print()
...
def download_fnpack():
    ...
    fnpack_path = os.path.join(PROJECT_ROOT, f"fnpack-{FNPACK_VERSION}-windows-amd64")

    if os.path.exists(fnpack_path):
        print("  -> fnpack 已存在，跳过下载")
        return fnpack_path

    print(f"  下载: {FNPACK_URL}")
    try:
        req = urllib.request.Request(FNPACK_URL, headers={"User-Agent": "Mozilla/5.0"})
        ...
        print(f"  !! 下载失败: {e}")
        print(f"  !! 请手动下载 {FNPACK_URL} 并放在项目根目录")
        return None
